...
def filter_words(tokens: List[str], word_counts=None, max_words: int = -1):
    ...
    tokens = (
        word
        for word in tokens
        if word[0] == "Ġ" and len([char for char in word[1:] if char.isalpha()]) >= 2
    )
...
def get_word_to_id_map(
    tokenizer: PreTrainedTokenizer, word_counts=None, max_words: int = -1
):
    """
...
class AutomaticVerbalizerSearch:
    def __init__(
        ...
        self.probs_list = [
            logits / np.expand_dims(np.sum(logits, axis=1), axis=1)
    ...
    def _get_candidates(self, num_candidates: int) -> Dict[str, List[str]]:
        ...
        return {
            label: [w for w, _ in scores[label].most_common(num_candidates)]
            for label in self.labels
        }
    ...
    def _get_top_words(
        ...
        return {
            label: [w for w, _ in scores[label].most_common(words_per_label)]
            for label in self.labels
        }

    @staticmethod
    def log_likelihood_ratio(
        predictions: np.ndarray, expected: np.ndarray, normalize: bool
    ) -> float:
        ...
        neg_score = np.sum(np.log(1 - predictions) * (1 - expected)) - np.sum(
            np.log(predictions) * (1 - expected)
        )
    ...
    def find_verbalizer(
        ...
        if score_fct == "random":
            return {
                label: random.sample(self.word2idx.keys(), words_per_label)
                for label in self.labels
            }
...
def main():
    ...
    parser.add_argument(
        "--model_type", default=None, type=str, required=True, help="The model type"
    )
    parser.add_argument(
        "--model_name_or_path",
        default=None,
    ...
    train_examples_per_label = (
        eq_div(args.train_examples, len(args.label_list))
    ...
    for pattern_id in args.pattern_ids:
        ...
        wrapper.preprocessor.pvp.convert_mlm_logits_to_cls_logits = (
            lambda mask, x, _=None: x[mask >= 0]
        )
    ...
    with open(
        os.path.join(args.output_dir, "verbalizers.json"), "w", encoding="utf8"
    ) as fh:
