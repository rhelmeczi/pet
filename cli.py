...
def load_sequence_classifier_configs(
    args,
) -> Tuple[WrapperConfig, pet.TrainConfig, pet.EvalConfig]:
    """
...
def main():
    ...
    parser.add_argument(
        "--priming", action="store_true", help="Whether to use priming for evaluation"
    )
    parser.add_argument(
        "--eval_set",
        choices=["dev", "test"],
    ...
    if args.split_examples_evenly:
        ...
        test_ex_per_label = (
            eq_div(args.test_examples, len(args.label_list))
