...
class MultiMaskTaskHelper(TaskHelper):
    ...
    def train_step(self, batch, **kwargs) -> Optional[torch.Tensor]:
        ...
        ), "train_step() for MultiMaskTaskHelper is only implemented for MLM models"
        ...
        correct_choice_token_ids = all_choice_token_ids[
            torch.arange(batch_size), all_labels
        ].permute(1, 0)

        wrong_choices_mask = torch.ones_like(all_choice_token_ids)
        wrong_choices_mask.scatter_(
            1, all_labels.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, max_seq_len), 0
        )
        wrong_choices_token_ids = all_choice_token_ids[wrong_choices_mask.bool()].view(
            batch_size, num_choices - 1, max_seq_len
        )
        ...
        loss_correct_choice = loss_fct(prediction_scores, correct_choice_token_ids).sum(
            dim=0
        )

        # compute hinge loss
        for wrong_choice_token_ids in wrong_choices_token_ids:
            loss_wrong_choice = loss_fct(prediction_scores, wrong_choice_token_ids).sum(
                dim=0
            )
    ...
    def eval_step(
        ...
        assert (
            self.wrapper.config.wrapper_type == "mlm"
        ), "eval_step() for MultiMaskTaskHelper is only implemented for MLM models"
        assert (
            batch["input_ids"].shape[0] == 1
        ), "eval_step() for MultiMaskTaskHelper is only implemented for batch_size=1"
    ...
    def add_features_to_dict(
        ...
        for feature in features:
            if len(feature.meta["choice_token_ids"]) != max_num_choices:
                raise ValueError(
                    f"The number of output choices must be identical for all examples, got "
                    f"{len(feature.meta['choice_token_ids'])} and {max_num_choices}"
                )

        feature_dict["choice_token_ids"] = torch.tensor(
            [f.meta["choice_token_ids"] for f in features], dtype=torch.long
        )
...
class CopaTaskHelper(TaskHelper):
    ...
    def get_sequence_classifier_inputs(self, example: InputExample) -> Dict[str, Any]:
        ...
        choice1, choice2 = lowercase_first(example.meta["choice1"]), lowercase_first(
            example.meta["choice2"]
        )
        question = example.meta["question"]
        joiner = "because" if question == "cause" else "so"
        text_a, text_b = " ".join([premise, joiner, choice1]), " ".join(
            [premise, joiner, choice2]
        )
        return self.wrapper.tokenizer.encode_plus(
            text_a,
            text_b,
