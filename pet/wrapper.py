...
class TransformerModelWrapper:
    ...
    def from_pretrained(cls, path: str) -> "TransformerModelWrapper":
        ...
        wrapper.preprocessor = PREPROCESSORS[wrapper.config.wrapper_type](
            wrapper,
            wrapper.config.task_name,
            wrapper.config.pattern_id,
            wrapper.config.verbalizer_file,
        )
        wrapper.task_helper = (
            ...
            for _, batch in enumerate(epoch_iterator):
                ...
                if lm_training:
                    ...
                    (
                        ...
                        unlabeled_batch["mlm_labels"],
                    ) = self._mask_tokens(lm_input_ids)
                    unlabeled_batch = {
                        k: t.to(device) for k, t in unlabeled_batch.items()
                    }
                ...
                loss = (
                    self.task_helper.train_step(batch, **train_step_inputs)
                    if self.task_helper
                    else None
                )

                if loss is None:
                    loss = TRAIN_STEP_FUNCTIONS[self.config.wrapper_type](self)(
                        batch, **train_step_inputs
                    )
    ...
    def eval(
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                logits = (
                    self.task_helper.eval_step(
                        batch, decoding_strategy=decoding_strategy
                    )
                    if self.task_helper
                    else None
                )
    ...
    def _generate_dataset(
        self, data: List[InputExample], labelled: bool = True, priming: bool = False
    ):
    ...
    def plm_train_step(
        self, labeled_batch: Dict[str, torch.Tensor], lm_training: bool = False, **_
    ):
        """Perform a PLM training step."""

        inputs = self.generate_default_inputs(labeled_batch)
        inputs["perm_mask"], inputs["target_mapping"] = (
            labeled_batch["perm_mask"],
            labeled_batch["target_mapping"],
        )
    ...
    def sequence_classifier_eval_step(
        self, batch: Dict[str, torch.Tensor]
    ) -> torch.Tensor:
